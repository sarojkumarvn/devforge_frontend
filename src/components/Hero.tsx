    ...
    <section className="relative flex min-h-screen items-center overflow-hidden px-4 pb-14 pt-28 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto w-full max-w-screen-xl">
        <motion.div className="mx-auto max-w-5xl text-center" initial="hidden" animate="visible" transition={{ staggerChildren: 0.12 }}>
          ...
          <motion.h1 variants={fadeUp} className="mx-auto mt-5 max-w-4xl text-balance font-heading text-[clamp(3rem,8vw,6rem)] font-medium leading-[0.92] tracking-[-0.05em]">
            Build. Share. <span className="text-primary">Connect.</span>
          </motion.h1>
          <motion.p variants={fadeUp} className="mx-auto mt-6 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground">
        ...
        <motion.div className="mx-auto mt-12 grid w-full max-w-5xl gap-4 md:grid-cols-[1.3fr_0.9fr]" initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.35 }}>
          <Card>
            ...
            <CardDescription className="text-pretty">Recent work from developers across DevForge.</CardDescription>
          </CardHeader>
            ...
            {['Published a Rust WASM profiler', 'Shipped a design system audit', 'Opened a GraphQL cache deep dive'].map((item, index) => (
              <div key={item}>
                {index > 0 && <Separator className="mb-3" />}
                  <div className="flex items-start gap-3"><Code2 className="text-primary" /><div><p className="font-medium text-pretty">{item}</p><p className="text-sm text-muted-foreground">Project update · just now</p></div></div>
              </div>
            ))}
          </CardContent>
          </Card>
          <Card className="hidden md:flex">
            <CardHeader>
              ...
              <CardDescription className="text-pretty">Focused spaces for builders.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-3">
              {[[ 'Frontend Ops', GitBranch ], [ 'AI Builders', MessageSquare ], [ 'Open Source', Code2 ]].map(([label, Icon]) => (
                <div key={label as string} className="flex items-center gap-3 rounded-xl bg-muted p-3"><Icon /><span className="font-medium">{label as string}</span></div>
              ))}
            </CardContent>
          </Card>
        </motion.div>
